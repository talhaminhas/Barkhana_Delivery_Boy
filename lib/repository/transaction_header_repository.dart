...
class TransactionHeaderRepository extends PsRepository {
 ...
 late TransactionHeaderDao _transactionHeaderDao;
  ...
  Future<dynamic> getAllNearestOrderList(
      StreamController<PsResource<List<TransactionHeader>>>
          pendingOrderListStream,
      bool isConnectedToInternet,
      String loginUserId,
    ...
    // Server Call
    if (isConnectedToInternet) {
      final PsResource<List<TransactionHeader>> _resource = await _psApiService
          .getAllNearestOrderList(loginUserId, limit, offset);
  ...
  Future<PsResource<ApiStatus>> postOrderAccept(
    ...
    final PsResource<ApiStatus> _resource =
        await _psApiService.postOrderAccept(jsonMap);
    if (_resource.status == PsStatus.SUCCESS) {
      return _resource;
    } else {
