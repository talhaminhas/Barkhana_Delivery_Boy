...
class PsApiService extends PsApi {
  ...
  /// Nearest Order
  ///
  Future<PsResource<List<TransactionHeader>>> getAllNearestOrderList(String loginUserId, int limit, int offset) async {
    final String url =
        '${PsUrl.ps_accept_order_for_delivery_boy_url}/api_key/${PsConfig.ps_api_key}/login_user_id/$loginUserId/limit/$limit/offset/$offset';

    return await getServerCall<TransactionHeader, List<TransactionHeader>>(TransactionHeader(), url);
  }

  ///
  /// Accept Order
  ///
  Future<PsResource<ApiStatus>> postOrderAccept(
      Map<dynamic, dynamic> jsonMap) async {
    const String url = '${PsUrl.ps_accept_order_url}/api_key/${PsConfig.ps_api_key}';
    return await postData<ApiStatus, ApiStatus>(
        ApiStatus(), url, jsonMap);
  }
